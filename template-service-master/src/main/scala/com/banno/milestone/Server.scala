...
import com.banno.milestone.admin.AdminService
import com.banno.milestone.admin.AdminService.AdminServiceExports
import com.banno.milestone.config.ConfigService
  ...
  def serve[F[_]](implicit Effect: Effect[F], EC: ExecutionContext): Stream[F, StreamApp.ExitCode] =
    for {
      ...
      exitCode <- Stream(
        ...
        .join(2)
        .concurrently(
          Stream.eval(Effect.delay(logger.info("milestone-project has started")))
        )
